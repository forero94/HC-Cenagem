...
const TABS_ORDER = ['resumen', 'miembros', 'arbol', 'complementarios', 'geneticos', 'fotos', 'evoluciones'];
function FamilyDetail({ family, members, evolutions, onBack, onAddEvolution, onUpdateMember, initialTab='resumen' }) {
...
export default function FamilyPage({ user = { email: 'genetista@cenagem.gob.ar' }, familyId }) {
  ...
  const handleAddEvolution = useCallback(
    async (memberId, texto) => {
      try {
        await addEvolution(memberId, texto, user?.email || 'registro');
      } catch (error) {
        console.error('No se pudo registrar la evolución', error);
      }
    },
    [addEvolution, user?.email],
  );
